...
def prev_evaluate(agent1, agent2, env, args):
    ...
    for _ in range(args.n_lookahead):
        ...
        phi1 = agent1.in_lookahead(memory_theta)
        phi2 = agent2.in_lookahead(memory_theta)
...
def train(agent1, agent2, env, log, tb_writer, args):
    while True:
        for iteration in range(200):
            ...
            agent1.out_lookahead(memory_phi)
            agent2.out_lookahead(memory_phi)
